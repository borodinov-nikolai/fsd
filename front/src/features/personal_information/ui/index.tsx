...
const Personal_information = () => {
 ...
 const handleSaveClick = async () => {
    ...
    const res = await putUserInfo({ user_id, body:{ name, phone_number, email, birthday, legal_entity} });
      ...
      <div className={styles.root} >
        ...
        <div className={styles.head} >
          <div className={styles.head_greeting} >
            <h1 className={styles.head_name} >Привет, <span>{(!isLoading && user_login)&& user_login[0].toUpperCase() + user?.username.slice(1)}</span></h1>
            <p className={styles.head_phoneNumber}>{!isLoading && user_phoneNumber}</p>
          </div>
          <div onClick={() => logout()} className={styles.head_leaveBtn}>Выйти из аккаунта</div>
        </div>
          ...
          <div className={styles.userInfo_personalaData}>
            <div className={styles.userInfo_itemsHolder} >
              <div className={styles.userInfo_name} >
                <p>Имя</p>
                <Input onChange={(e) => setName(e.target.value)} defaultValue={user_name} placeholder='Имя' />
              </div>
              <div className={styles.userInfo_birthday} >
                <p>Дата рождения</p>
                <Input onChange={(e) => setBirthday(e.target.value)} defaultValue={user_birthday} placeholder='Дата' />
              </div>
            ...
            <div className={styles.userInfo_itemsHolder} >
              <div className={styles.userInfo_number} >
                <p>Телефон</p>
                <Input defaultValue={user_phoneNumber} onChange={(e) => setPhone_Number(e.target.value)} placeholder='Телефон' />
              </div>
